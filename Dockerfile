...
#  && apt-get install -y --no-install-recommends logstash=$LOGSTASH_VERSION \
  && wget -nv -O /root/logstash_2.3.1-1_all.deb  https://download.elastic.co/logstash/logstash/packages/debian/logstash_2.3.1-1_all.deb
RUN dpkg -i /root/logstash_2.3.1-1_all.deb; exit 0
RUN apt-get -f install -y --no-install-recommends \
  && dpkg -i /root/logstash_2.3.1-1_all.deb \
  && rm -rf /var/lib/apt/lists/*
